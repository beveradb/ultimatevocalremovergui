...
class SeperateAttributes:
    def __init__(
        ...
        if model_data.process_method == MDX_ARCH_TYPE:
            ...
            self.logger.debug(
                f"Model params: primary_model_name={self.primary_model_name}, primary_sources={self.primary_sources}, primary_stem={self.primary_stem}, secondary_stem={self.secondary_stem}"
            )
            self.logger.debug(f"Model params: is_mdx_ckpt={self.is_mdx_ckpt}, is_denoise={self.is_denoise}, is_denoise_model={self.is_denoise_model}, is_mdx_c_seg_def={self.is_mdx_c_seg_def}")
            self.logger.debug(f"Model params: batch_size={self.mdx_batch_size}, compensate={self.compensate}, segment_size={self.mdx_segment_size}, dim_f={self.dim_f}, dim_t={self.dim_t}")
            self.logger.debug(f"Model params: n_fft={self.n_fft}, chunks={self.chunks}, margin={self.margin}, adjust={self.adjust}, dim_c={self.dim_c}, hop={self.hop}")
    ...
    def process_vocal_split_chain(self, sources: dict):
        def is_valid_vocal_split_condition(master_vocal_source):
            ...
            conditions = [isinstance(master_vocal_source, np.ndarray), self.vocal_split_model, not self.is_ensemble_mode, not self.is_karaoke, not self.is_bv_model]
        ...
        if is_valid_vocal_split_condition(master_vocal_source):
            process_chain_model(self.vocal_split_model, self.process_data, vocal_stem_path=self.master_vocal_path, master_vocal_source=master_vocal_source, master_inst_source=master_inst_source)

    def process_secondary_stem(self, stem_source, secondary_model_source=None, model_scale=None):
        if not self.is_secondary_model:
            self.logger.debug(f"process_secondary_stem: is_secondary_model is False")
            if self.is_secondary_model_activated and isinstance(secondary_model_source, np.ndarray):
                self.logger.debug(f"process_secondary_stem: is_secondary_model_activated is True, running average_dual_sources")
    ...
    def write_audio(self, stem_path: str, stem_source, samplerate, stem_name=None):
        ...
        def save_audio_file(path, source):
            self.logger.debug(f"save_audio_file normalizing source with is_normalization: {self.is_normalization}")
            source = spec_utils.normalize(source, self.is_normalization)
            sf.write(path, source, samplerate, subtype=self.wav_type_set)
            self.logger.debug(f"save_audio_file writing source to path: {path}")

            if is_not_ensemble:
                self.logger.debug(f"save_format with path: {path}, save_format: {self.save_format}, mp3_bit_set: {self.mp3_bit_set}")
        ...
        def save_with_message(stem_path, stem_name, stem_source):
            is_deverb = self.is_deverb_vocals and (
                self.deverb_vocal_opt == stem_name or (self.deverb_vocal_opt == "ALL" and (stem_name == VOCAL_STEM or stem_name == LEAD_VOCAL_STEM_LABEL or stem_name == BV_VOCAL_STEM_LABEL))
            )
...
class SeperateMDX(SeperateAttributes):
    def seperate(self):
        self.logger.debug("SeperateMDX starting the separation process...")
        ...
        if not self.is_primary_stem_only:
            ...
            if not isinstance(self.secondary_source, np.ndarray):
                ...
                if self.is_invert_spec:
                    self.logger.debug("Inverting secondary stem using spectogram as is_invert_spec is set to True")
    ...
    def initialize_model_settings(self):
        ...
        self.logger.debug(f"Model input params: n_fft={self.n_fft} hop_length={self.hop} dim_f={self.dim_f}")
    ...
    def demix(self, mix, is_match_mix=False):
        ...
        if overlap == DEFAULT:
            ...
            self.logger.debug(f"Step size for processing chunks: {step} as overlap is set to default.")
        else:
        ...
        # Correcting the pitch if requested
...
class SeperateDemucs(SeperateAttributes):
    def seperate(self):
        ...
        if is_no_cache:
            if self.demucs_version == DEMUCS_V1:
                ...
                self.demucs = _gm(name=os.path.splitext(os.path.basename(self.model_path))[0], repo=Path(os.path.dirname(self.model_path)))
                ...
                if self.is_secondary_model_activated and not self.is_secondary_model and not stem_value >= 4:
                    if self.secondary_model_4_stem[stem_value]:
                        ...
                        stem_source_secondary = process_secondary_model(
                            self.secondary_model_4_stem[stem_value], self.process_data, main_model_primary_stem_4_stem=stem_name, is_source_load=True, is_return_dual=False
                        )
    ...
    def demix_demucs(self, mix):
        ...
        with torch.no_grad():
            if self.demucs_version == DEMUCS_V1:
                sources = apply_model_v1(self.demucs, mix_infer.to(self.device), self.shifts, self.is_split_mode, set_progress_bar=self.set_progress_bar)
            elif self.demucs_version == DEMUCS_V2:
                sources = apply_model_v2(self.demucs, mix_infer.to(self.device), self.shifts, self.is_split_mode, self.overlap, set_progress_bar=self.set_progress_bar)
            else:
                sources = apply_model(
                    self.demucs,
                    mix_infer[None],
...
def process_secondary_model(
    secondary_model: ModelData, process_data, main_model_primary_stem_4_stem=None, is_source_load=False, main_process_method=None, is_pre_proc_model=False, is_return_dual=True, main_model_primary=None
):
    ...
    if secondary_model.process_method == VR_ARCH_TYPE:
        seperator = SeperateVR(
            secondary_model, process_data, main_model_primary_stem_4_stem=main_model_primary_stem_4_stem, main_process_method=main_process_method, main_model_primary=main_model_primary
        )
...
def process_chain_model(secondary_model: ModelData, process_data, vocal_stem_path, master_vocal_source, master_inst_source=None):
    process_iteration = process_data["process_iteration"]
    process_iteration()
